...
### Changed
- **DocuDesk register configuration version bumped 5.1.0 → 7.0.0** to trigger OpenRegister's `imported_config_docudesk_version` gate so new schemas and the `appendOnly` flag on `signingAuditEntry` are imported on app upgrade. (`entity-publication-policies`, #287, #289)
