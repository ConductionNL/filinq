 ...
 *   get <key>
 *   find <substring>
 *   add <key> --value <lang>=<text> [--value ...] [--plural <lang>=<text> ...] [--locales=a,b] [--force]
 *   set <key> --locale=<lang> --value=<text> [--plural=<text> ...]
 *   rm <key> [--force]
...
/**
 * Parse a list of "lang=text" strings into a { lang: [text, ...] } map.
 * Unlike parseValuePairs, repeated entries for the same locale accumulate —
 * one per extra plural form, in the order given. Used for `add --plural`, where
 * a key may have more than one plural form (languages with nplurals > 2).
 */
function parsePluralPairs(pairs) {
	const out = {}
	for (const p of pairs) {
		const eq = p.indexOf('=')
		if (eq === -1) {
			throw new Error(`--plural expects 'lang=text', got '${p}'`)
		}
		const lang = p.slice(0, eq)
		const text = p.slice(eq + 1)
		if (!lang) throw new Error(`--plural missing locale: '${p}'`)
...
function cmdAdd(args) {
	...
	try {
		...
		pluralMap = parsePluralPairs(opts.plural || [])
	} catch (err) {
		fail(`add: ${err.message}`)
	}

	// A key is pluralized (array value) iff any --plural form was supplied. When
	// it is, the stored value per locale is [singular, ...plural forms].
	const isPlural = Object.keys(pluralMap).length > 0
	...
	// locales and a string in others (which breaks n() at runtime).
	if (isPlural) {
	...
	for (const e of entries) {
		if (!targetLocales.has(e.locale)) continue
		const next = { ...e.translations, [key]: valueFor(e.locale) }
		toWrite.push({ ...e, translations: next })
	}
	writeAll(toWrite)
	for (const e of toWrite) {
		const v = valueFor(e.locale)
		console.log(`${e.locale}.js\t${Array.isArray(v) ? JSON.stringify(v) : v}`)
	}
}
...
function cmdSet(args) {
	...
	// --plural makes (or keeps) the value an array: [singular, ...plural forms].
	// Without --plural we refuse to overwrite an existing array, so a plural key
	// can't be silently downgraded to a single string (which breaks n()).
	...
	const nextValue = hasPlural
		? [opts.value, ...(Array.isArray(pluralForms) ? pluralForms : [pluralForms])]
		: opts.value
...
function cmdHelp() {
	const text = [
		...
		'      [--plural <lang>=<text> ...]         plural form(s) per locale → [singular, ...plurals]',
		'      [--locales=a,b] [--force]',
		'  set <key> --locale=<lang> --value=<text> update a single locale',
		'      [--plural=<text> ...]                plural form(s) → [singular, ...plurals]',
