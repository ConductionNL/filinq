...
export const useStandingConsentStore = defineStore(
	'standingConsent',
	{
		state: () => ({
			standingConsents: [],
		...
		getters: {
			standingConsentStats: (state) => ({
				...
				inactive: state.standingConsents.filter(s => s.active === false).length,
			}),
		},
		actions: {
			...
			async updateStandingConsent(id, data) {
				...
				try {
					...
					const idx = this.standingConsents.findIndex(s => (s['@self']?.id || s.id || s.uuid) === id)
					if (idx !== -1) {
			...
			async deleteStandingConsent(id) {
				...
				try {
					...
					this.standingConsents = this.standingConsents.filter(s => (s['@self']?.id || s.id || s.uuid) !== id)
				} catch (err) {
