...
export const useCustomDictionaryStore = defineStore(
	'customDictionary',
	{
		state: () => ({
			dictionaries: [],
		...
		actions: {
			...
			async deleteDictionary(id) {
				...
				try {
					...
					this.dictionaries = this.dictionaries.filter(d => recordId(d) !== id)
				} catch (err) {
			...
			async fetchTerms(dictionaryId) {
				...
				this.error = null
				try {
					const response = await axios.get(generateUrl(`${baseUrl}/${dictionaryId}/terms`))
			...
			async createTerm(dictionaryId, data) {
				try {
					const response = await axios.post(generateUrl(`${baseUrl}/${dictionaryId}/terms`), data)
			...
			async deleteTerm(dictionaryId, termId) {
				try {
					await axios.delete(generateUrl(`${baseUrl}/${dictionaryId}/terms/${termId}`))
			...
			async importTerms(dictionaryId, payload) {
				...
				try {
					...
					if (payload.file) {
						...
						response = await axios.post(generateUrl(`${baseUrl}/${dictionaryId}/import`), formData)
					} else {
						response = await axios.post(generateUrl(`${baseUrl}/${dictionaryId}/import`), {
