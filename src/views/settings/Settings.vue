<template>
	<CnAdminSettingsShell
		...
		<NcSettingsSection
			...
			<div v-else>
				<div
					v-for="entityType in grondslagEntityTypes"
					:key="entityType"
					class="setting-item">
					<div class="setting-label">
						{{ entityType }}
				...
				<div v-for="type in objectTypes" :key="type">
					...
					<div class="selectionContainer">
						...
						<NcButton
							...
							<template #icon>
								<NcLoadingIcon
									v-if="loading || sections[type].loading"
									:size="20" />
								<Plus v-else :size="20" />
							</template>
							{{ t('filinq', 'Save') }}
...
export default {
	...
	methods: {
		...
		selectedBasesFor(entityType) {
			...
			return this.grondslagBaseOptions.filter((option) =>
			...
			fetch('/index.php/apps/filinq/api/settings', { method: 'GET' })
				...
				.then((data) => {
					...
					this.settings.publication_objection_period_days =
						data.publication_objection_period_days ?? 28
					this.settings.enable_language_detection =
						data.enable_language_detection ?? true
					this.settings.enable_keyword_extraction =
					...
					this.objectTypes.forEach((type) => {
						newSections[type] = {
							selectedRegister:
								this.availableRegistersOptions.options.find(
									(option) =>
										option.value
										=== data.configuration?.[`${type}_register`],
								) || '',
							selectedSchema: '',
							loading: false,
						}
						...
						if (
							data.configuration?.[`${type}_register`]
							&& data.configuration?.[`${type}_schema`]
						) {
		...
		saveAll() {
			...
			const ocrLangs =
				Object.entries(this.ocrLanguages)
			...
			this.objectTypes.forEach((type) => {
				...
				payload[`${type}_schema`] =
					this.sections[type]?.selectedSchema?.value || ''
		...
		openProcessingExport() {
			const registers = ['document', 'signing', 'dossier', 'consent']
				...
				.join('&')
			this.openLink(
				`/index.php/apps/openregister/api/avg/verwerkingen?${registers}`,
				'_blank',
			)
		},
	},
