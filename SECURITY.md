...
Example — fetch the latest filinq SBOM:

```bash
curl -sL https://github.com/ConductionNL/filinq/releases/latest/download/sbom.cdx.json | jq .
```

Example — fetch the SBOM for a specific historical release:

```bash
curl -sL https://github.com/ConductionNL/filinq/releases/download/v1.0.0/sbom.cdx.json | jq .
```
