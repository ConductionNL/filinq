...
activities (*verwerkingsregister*). Filinq performs systematic, large-scale
...
abstraction decision). Filinq is a **thin consumer**: it contributes the
document-processing activity catalogue, the grondslag/retention mapping, and an
admin UI window onto the platform register. Filinq ships **no** aggregation
service, export controller, or register template of its own (ADR-022).

## What Filinq contributes

### The four processing activities (catalogue)

Filinq declares its four processing activities as `x-openregister-processing`
catalogue annotations in `lib/Settings/filinq_register.json`. Each annotation
...
|---|---|---|---|---|
| Anonymisation of documents | `filinq-anonymisation` | `anonymizationLink` | `filinq.anonymiser` | P7Y (selectielijst category to be confirmed) |
| OCR text extraction | `filinq-ocr` | `generatedDocument` | `filinq.ocr.tesseract` | not declared |
| Document metadata enrichment | `filinq-metadata-enrichment` | `base` | `filinq.metadata.enricher` | not declared |
| Digital document signing | `filinq-signing` | `signingAuditEntry` | `filinq.signing` | P10Y (Archiefwet 1995 selectielijst cat. 5.1.3) |
...
The Filinq admin settings (`/settings/admin/filinq`) gain an **AVG Art. 30
...
- deep-links to OpenRegister's per-access processing log scoped to Filinq's
...
Filinq's contribution (catalogue annotations + read-logging opt-in + admin
...
| (d) Recipients | OpenRegister (out of scope for Filinq) |
...
- Register annotations: `lib/Settings/filinq_register.json`
