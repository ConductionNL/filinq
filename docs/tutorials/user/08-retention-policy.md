...
description: Attach a retention policy to a document so Filinq schedules its review-and-delete date and warns you before it expires.
---

# Apply a retention policy

A **retention policy** captures *how long* a document must be kept (e.g. *seven years for invoices*, *one year for application drafts*) and *what should happen* when that period expires (delete, archive, anonymise). Attaching a policy to a document hands the lifecycle to Filinq: it schedules the review date, surfaces upcoming expirations on the dashboard, and runs the configured action when the timer fires.

## Goal

By the end you will have picked a retention policy from the instance library, attached it to one document, and verified that Filinq recorded the review-and-delete date.

## Prerequisites

- You have completed [Upload a document](./02-upload-document.md).
- The instance has at least one retention policy defined (admins manage these in **Settings → Filinq → Retention policies**).
...
3. Submit. Filinq attaches the policy to the document, computes the **Review date** from the policy's duration plus today, and writes it to the document object.
...
| Symptom | Fix |
|---|---|
| Policy picker is empty | No retention policies are defined on this instance — ask an admin to create one in **Settings → Filinq → Retention policies**. |
...
| Review date in the past after applying | The chosen policy's duration is shorter than the document's age (e.g. *one year* policy on a two-year-old document). Filinq surfaces this as a *Review now* badge; the expiration action will fire on the next scheduled run. |
