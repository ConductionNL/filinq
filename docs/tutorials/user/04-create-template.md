...
description: Define a reusable document template with placeholder fields that Filinq can render into a finished document later.
---

# Create a template

A **Template** is a reusable document skeleton with named placeholder fields. Once a template exists, you (or any other Filinq flow) can render it into a finished document by filling those placeholders — useful for letters, contracts, receipts, GDPR responses, anything you produce more than once.
...
- You have a DOCX or ODT file ready that uses Filinq's placeholder syntax (`\{\{ fieldName \}\}`).
...
4. Upload the DOCX or ODT file in the **Source** tab. Filinq parses it, lists detected placeholders, and warns about any that aren't declared in the **Fields** tab (or declared fields that don't appear in the source).
...
| Symptom | Fix |
|---|---|
| Source upload says "no placeholders detected" | Your DOCX uses the wrong delimiters — Filinq expects `\{\{ name \}\}`, not `{name}` or `[name]`. Update the source. |
...
| Template detail won't open after creation | The OpenRegister write failed at create time (often a permissions issue). Check **Settings → Filinq → Logs** for the underlying error. |
