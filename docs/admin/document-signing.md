...
sidebar_position: 3
description: Configure DocuDesk's signing providers, register/schema bindings, default signature levels, and signing-request expiry.
keywords:
  - signing
  - administrator
  - configuration
...
operating DocuDesk's signing workflow. It is the operational counterpart to the
...
- OpenRegister is installed and the DocuDesk register/schema set has been
  imported (DocuDesk's repair step `InitializeRegister` handles this on first
  enable).
...
`docudesk.signing_provider`. Two providers ship in-app:
...
|---|---|---|
| Signing request | `docudesk.signingRequest_register` | `docudesk.signingRequest_schema` |
| Signer record   | `docudesk.signerRecord_register`   | `docudesk.signerRecord_schema`   |
| Signing session | `docudesk.signingSession_register` | `docudesk.signingSession_schema` |
| Audit entry     | `docudesk.signingAuditEntry_register` | `docudesk.signingAuditEntry_schema` |
...
| Key | Purpose |
|---|---|
| `docudesk.validsign_base_url` | API base URL of the ValidSign tenant. |
| `docudesk.validsign_api_token` | Bearer token used in `Authorization` headers. |
| `docudesk.validsign_webhook_secret` | HMAC secret used to verify callback signatures. |
...
- **Audit trail** — every state-changing operation
  ...
  OpenRegister.
- **Notifications** — DocuDesk emits notifications via the canonical
  ...
  `lib/Settings/docudesk_register.json` and re-import.
...
| Requests stay `pending` after `expiresAt` | `SigningExpirationJob` not scheduled | Ensure NC cron runs at least once a day; verify `background-jobs` block in `appinfo/info.xml` was picked up by re-running `occ app:enable docudesk`. |
...
- ADR-022 (apps-consume-or-abstractions) — explains why DocuDesk's signing
