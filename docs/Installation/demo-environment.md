...
This page gets a working Filinq running on your own machine in two commands. You end with document generation and anonymisation over registry objects.
...
```bash
curl -fsSLO https://raw.githubusercontent.com/ConductionNL/filinq/development/filinq-compose.yaml
```
...
==> installing filinq <version>
==> apps present: filinq integriq openregister thematiq
```
...
```bash
curl -s http://localhost:8603/status.php
```
...
| What | Where |
| --- | --- |
| **Filinq** | [http://localhost:8603/apps/filinq/](http://localhost:8603/apps/filinq/) |
| Admin interface | [http://localhost:8603](http://localhost:8603) — `admin` / `admin` |
...
| `filinq` | The app this page is about. |

That OpenRegister dependency is **not declared** in `appinfo/info.xml` — no app in the fleet declares an `<app>` dependency — so nothing stops the App Store from installing filinq without it. It would then load, find no register to attach to, and show you an empty app rather than an error. The compose file encodes the dependency the manifest does not.
...
curl -s -u admin:admin "http://localhost:8603/apps/openregister/api/registers" | head -c 300
```
...
```bash
# Stop, keep the data
docker compose -f filinq-compose.yaml down

# Stop and delete everything, including the database
docker compose -f filinq-compose.yaml down -v
```
...
**Everything returns 404 or a maintenance page after a restart.** Nextcloud is waiting for an upgrade. Run `docker compose -f filinq-compose.yaml exec -u www-data nextcloud php occ upgrade`.
