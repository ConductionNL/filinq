...
async function provision(page: Page, req: APIRequestContext): Promise<Fixture> {
	...
	const dict = await req.post(`${API}/custom-dictionaries`, {
		headers: jsonHeaders(token),
		data: {
			label: `${TEST_PREFIX}-review-dict`,
			description: 'entity-review fixture',
			matchMode: 'caseInsensitive',
			active: true,
		},
	})
	expect(dict.status(), `POST ${API}/custom-dictionaries must answer 201`).toBe(
		201,
	)
	...
	expect(
		batch.status(),
		`POST ${API}/anonymization/batch/folder must answer 200`,
	).toBe(200)
	...
	expect(
		...
		`the folder-batch must have found the two seeded files in ${FOLDER}. `
			+ `Response: ${JSON.stringify(batchBody).slice(0, 400)}`,
	).toBe(2)
	...
	expect(
		extract.status(),
		`POST .../batch/${batchId}/extract must answer 200`,
	).toBe(200)
	expect(
		(await extract.json()).batchStatus,
	...
	expect(rule.status(), `POST ${API}/policy/prohibitions must answer 201`).toBe(
		201,
	)
...
test.describe('anonymization-entity-review — consolidated entities endpoint', () => {
	test('the endpoint returns deduplicated entities carrying every declared field', async ({
		page,
		request,
	}) => {
		...
		for (const e of list) {
			for (const field of [
				...
				expect(
					Object.keys(e),
					`every entity entry must carry "${field}"`,
				).toContain(field)
			}
		}
		...
		expect(
			a,
			`"${TERM_A}" occurs in two files and must appear exactly once`,
		).toHaveLength(1)
		expect(
			a[0].fileCount,
			`"${TERM_A}" is in two documents, so fileCount must be 2`,
		).toBe(2)
		const b = list.filter((e) => e.value === TERM_B)
		expect(
			b[0].fileCount,
			`"${TERM_B}" is in one document, so fileCount must be 1`,
		).toBe(1)

		// SORTED BY CONFIDENCE DESCENDING.
		const conf = list.map((e) => Number(e.highestConfidence))
		expect(conf, `confidences in response order: ${conf.join(', ')}`).toEqual(
			[...conf].sort((x, y) => y - x),
		)
	})

	test('an entity no prohibition rule matches reports prohibitionMatch null', async ({
		page,
		request,
	}) => {
		// @e2e openspec/specs/anonymization-entity-review/spec.md#entity-with-no-prohibition-match-returns-null
		const f = await provision(page, request)
		const list = (await entities(request, f)).entities as Array<
			Record<string, unknown>
		>
		const unmatched = list.find((e) => e.value === TERM_B)
		expect(
			unmatched,
			`positive control — "${TERM_B}" must be in the list at all`,
		).toBeDefined()
		expect(
			unmatched!.prohibitionMatch,
			`no prohibition rule names "${TERM_B}" (the only rule names "${TERM_A}"), so its match must be null`,
		).toBeNull()
	})

	test('a matching prohibition rule is reported with ruleId, ruleName and highConfidence', async ({
		page,
		request,
	}) => {
		...
		expect(
			matched,
			`positive control — "${TERM_A}" must be in the list at all`,
		).toBeDefined()

		const pm = matched!.prohibitionMatch as Record<string, unknown> | null
		expect(
			pm,
			`an active prohibition rule names "${TERM_A}", so its match must NOT be null`,
		).not.toBeNull()
		expect(
			pm!.ruleId,
			'the match must carry the id of the rule that fired',
		).toBe(f.ruleId)
		expect(pm!.ruleName, 'the match must carry the rule primaryName').toBe(
			TERM_A,
		)
		...
		expect(
			matched!.fileCount,
			'this assertion is only meaningful across >1 file',
		).toBe(2)
		expect(
			pm!.highConfidence,
	...
	test('files that belong to no dossier yield an empty suggestedBases', async ({
		page,
		request,
	}) => {
		...
		expect(
			list.length,
			'positive control — the list must be populated',
		).toBeGreaterThan(0)
		for (const e of list) {
			expect(
				e.suggestedBases,
	...
	test('the response is a strict superset of the pre-change shape', async ({
		page,
		request,
	}) => {
		...
		for (const e of list) {
			...
			expect(
				typeof e.highestConfidence,
				'highestConfidence must still be a number',
			).toBe('number')
			expect(typeof e.fileCount, 'fileCount must still be a number').toBe(
				'number',
			)
			expect(typeof e.included, 'included must still be a boolean').toBe(
				'boolean',
			)
		}
	})

	test('omitting minConfidence applies no confidence filtering at all', async ({
		page,
		request,
	}) => {
...
async function driveToReview(page: Page): Promise<void> {
	await page.goto(await appUrl(page, 'folder-anonymization'), {
	...
	await expect(
		analyze,
		'the Analyze action must enable once a path is typed',
	).toBeEnabled()
	await analyze.click()

	// The review step is what the excluded spec called unbuilt.
	await expect(
		page.getByRole('heading', { name: 'Review Entities' }),
		'the folder analysis must reach the review step',
	).toBeVisible({ timeout: 120_000 })
	await expect(
		page.locator('.entity-review table.entity-table'),
		'the entity review table must render',
	).toBeVisible()
	...
	await expect(
		page.locator('.entity-review table.entity-table tbody tr'),
...
function row(page: Page, value: string) {
	return page
		.locator('.entity-review table.entity-table tbody tr')
...
function bulk(page: Page, label: string) {
	return page
		.locator('.entity-review .bulk-actions')
		.getByRole('button', { name: label, exact: true })
}

test.describe('anonymization-entity-review — review table UI', () => {
	test('the review table renders a row per entity plus the summary bar', async ({
		page,
		request,
	}) => {
		...
		await expect(
			r.locator('input[type="checkbox"]').first(),
			'each row has an include checkbox',
		).toBeVisible()
		...
		await expect(
			r.locator('.badge'),
			'each row has a type badge naming its entity type',
		).toHaveText(/CUSTOM_DICTIONARY|Custom dictionary/i)
		await expect(r, 'each row shows a confidence percentage').toContainText(
			'100.0%',
		)

		await expect(
			page.locator('.entity-review .summary-bar'),
			'the summary bar reports selected / total / file counts',
		).toContainText('of 2 entities selected')
	})

	test('the search box filters rows by value substring', async ({
		page,
		request,
	}) => {
		...
		await expect(
			row(page, TERM_A),
			`"${TERM_A}" contains "${NEEDLE}" and must remain listed`,
		).toHaveCount(1)
		await expect(
			row(page, TERM_B),
			`"${TERM_B}" does not contain "${NEEDLE}" and must be filtered out`,
		).toHaveCount(0)
	...
	test('a previously excluded entity can be included from the table', async ({
		page,
		request,
	}) => {
		...
		await expect(
			box,
			'precondition — the entity must be excluded before it can be re-included',
		).not.toBeChecked()
		await expect(page.locator('.entity-review .summary-bar')).toContainText(
			'0 of 2 entities selected',
		)

		await box.check()
		await expect(
			box,
			'checking the box must set included=true in the store',
		).toBeChecked()
		await expect(page.locator('.entity-review .summary-bar')).toContainText(
			'1 of 2 entities selected',
		)
	})

	test('unchecking an entity removes it from the payload the anonymise call sends', async ({
		page,
		request,
	}) => {
		...
		await expect
			.poll(() => payloads.length, {
				message:
					'the Anonymize action must actually POST the approved entity list',
				timeout: 60_000,
			})
			.toBeGreaterThan(0)
		...
		expect(
			sent,
			`positive control — the INCLUDED entity must be in the payload. Sent: ${sent.slice(0, 400)}`,
		).toContain(TERM_A)
		expect(
			sent,
			`the UNCHECKED entity must NOT be sent. Sent: ${sent.slice(0, 400)}`,
		).not.toContain(TERM_B)
	})

	test('Select All Visible affects only the filtered rows; Deselect All clears them', async ({
		page,
		request,
	}) => {
		...
		await expect(
			row(page, TERM_A),
			'positive control — the visible row must be there',
		).toHaveCount(1)
		await bulk(page, 'Select All Visible').click()
		await expect(
			page.locator('.entity-review .summary-bar'),
			'only the one visible entity may be selected',
		).toContainText('1 of 2 entities selected')

		await page.getByRole('textbox', { name: 'Search entities' }).fill('')
		await expect(
			row(page, TERM_B).locator('input[type="checkbox"]').first(),
			'the row hidden behind the filter must NOT have been selected',
		).not.toBeChecked()
