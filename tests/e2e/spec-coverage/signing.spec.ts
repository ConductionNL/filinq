...
test.describe('document-signing — signing requests list UI', () => {
	test('Signing Requests page renders heading and a list or empty-state', async ({ page }) => {
		// @e2e openspec/specs/document-signing/spec.md#list-all-signing-requests
	...
	test('signing requests table exposes status / level / mode columns when populated', async ({ page }) => {
		// @e2e openspec/specs/document-signing/spec.md#view-signing-request-status
		await go(page, 'signing')
		const table = page.locator('#content table, .app-content table').first()
		if (await table.isVisible().catch(() => false)) {
			...
			await expect(page.locator('.empty-content, [class*="empty-content"]')
				.filter({ hasText: 'No signing requests' }).first()).toBeVisible()
		}
	})

	test('Signing Requests is reachable via the left navigation', async ({ page }) => {
		// @e2e openspec/specs/document-signing/spec.md#list-all-signing-requests
		await go(page, '')
