...
describe('buildWebdavUrl', () => {
	it('builds a DAV URL from a user-relative path', () => {
		expect(buildWebdavUrl('/DocuDesk/foo.pdf'))
			.toBe('http://localhost/remote.php/dav/files/admin/DocuDesk/foo.pdf')
	})

	it('strips the Nextcloud-internal /<uid>/files prefix', () => {
		expect(buildWebdavUrl('/admin/files/DocuDesk/foo.pdf'))
			.toBe('http://localhost/remote.php/dav/files/admin/DocuDesk/foo.pdf')
	})

	it('prepends a leading slash to a bare relative path', () => {
		expect(buildWebdavUrl('DocuDesk/bar.txt'))
			.toBe('http://localhost/remote.php/dav/files/admin/DocuDesk/bar.txt')
	})
	...
	it('maps the exact internal prefix to root', () => {
		expect(buildWebdavUrl('/admin/files'))
			.toBe('http://localhost/remote.php/dav/files/admin/')
	})

	it('throws when there is no authenticated user', () => {
		__setCurrentUser(null)
		expect(() => buildWebdavUrl('/DocuDesk/foo.pdf')).toThrow('User not authenticated')
	})
})
