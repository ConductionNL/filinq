 ...
 *
 * A migration that moves data and is never exercised is the worst thing in the
 * repo to leave untested, and the coverage ratchet was right to fail the PR that
 * added it.
 *
 ...
 * Doctrine\DBAL\ParameterType::STRING, and these apps ship doctrine/deprecations
 ...
 * Throwable around its information_schema reads, so the thrown error surfaced as
...
class RenameDutchColumnsTest extends TestCase {
	...
	private function step(
		...
		// The register lookup: one executeQuery per slug prefix.
	...
	public function testRenamesWhenOnlyTheDutchColumnExists(): void {
		...
		$step = $this->step([7], [$table], [$table => ['id', 'zaak_id']]);
		$step->run($this->repairOutput());
		$this->assertCount(1, $this->executed);
		$this->assertStringContainsString('RENAME COLUMN "zaak_id" TO "case_id"', $this->executed[0]);
	...
	public function testBackfillsWhenBothColumnsExist(): void {
		$table = 'oc_openregister_table_7_3';
		$step = $this->step([7], [$table], [$table => ['id', 'zaak_id', 'case_id']]);
		$step->run($this->repairOutput());
		$this->assertCount(1, $this->executed);
		$this->assertStringContainsString('UPDATE', $this->executed[0]);
		$this->assertStringContainsString('"case_id" = "zaak_id"', $this->executed[0]);
		$this->assertStringContainsString('"case_id" IS NULL', $this->executed[0]);
		$this->assertStringNotContainsString('DROP', $this->executed[0]);
	}//end testBackfillsWhenBothColumnsExist()

	/**
	 * A table without the Dutch column is left alone.
	 *
	 * @return void
	 */
	public function testTableWithoutTheDutchColumnIsUntouched(): void {
		$table = 'oc_openregister_table_7_3';
		$step = $this->step([7], [$table], [$table => ['id', 'case_id']]);
		$step->run($this->repairOutput());
		$this->assertSame([], $this->executed);
	...
	public function testNonNumericSuffixIsNotAShard(): void {
		...
		$step = $this->step([7], [$table], [$table => ['zaak_id']]);
		$step->run($this->repairOutput());
		$this->assertSame([], $this->executed);
	...
	public function testFailedStatementIsLoggedAndDoesNotThrow(): void {
		$table = 'oc_openregister_table_7_3';
		$step = $this->step([7], [$table], [$table => ['zaak_id']], failOn: 'RENAME COLUMN');
		$step->run($this->repairOutput());
		$this->assertSame([], $this->executed);
	...
	public function testAllShardsOfTheRegisterAreMigrated(): void {
		...
		$step = $this->step([7], [$a, $b], [$a => ['zaak_id'], $b => ['zaak_id']]);
		$step->run($this->repairOutput());
	...
	public function testSummaryReportsCounts(): void {
		$a = 'oc_openregister_table_7_1';
		$b = 'oc_openregister_table_7_2';
		$step = $this->step([7], [$a, $b], [$a => ['zaak_id'], $b => ['zaak_id', 'case_id']]);
		$step->run($this->repairOutput());
