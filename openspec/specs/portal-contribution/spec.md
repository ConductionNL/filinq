...
DocuDesk contributes a `data-subject` and a `signer` section to **portaliq**,
...
(`OCA\DocuDesk\Portal\PortalContributionProvider`, duck-typed by FQCN — inert
...
### Requirement: DocuDesk ships the ADR-046 portal contribution (REQ-DDPORT-000)
...
`OCA\DocuDesk\Portal\PortalContributionProvider` class (duck-typed by FQCN,
...
- GIVEN DocuDesk installed with portaliq present
- WHEN portaliq resolves `OCA\DocuDesk\Portal\PortalContributionProvider` by FQCN and calls `getAudiences()` / `getContribution()`
...
- @e2e exclude backend-only contribution class rendered inside portaliq, not DocuDesk — covered by PHPUnit (tests/unit/Portal/PortalContributionProviderTest.php)
