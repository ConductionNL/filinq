...
> @e2e exclude Pure backend OpenRegister data-model definition — register/schema JSON structure, report/template/entity schemas, pre-seeded sample objects, and the boot register-loading gap. No browser-driven UI surface (data is consumed by other DocuDesk views, each covered by their own spec). Verified by PHPUnit register-import tests and the Newman docudesk-api object collection.
