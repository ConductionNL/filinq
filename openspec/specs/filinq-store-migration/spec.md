...
# Specification — `filinq-store-migration` (Phase 1, side-by-side)
...
Phase 1 (side-by-side) adoption of the shared `@conduction/nextcloud-vue` `useObjectStore` as filinq's canonical OR-CRUD store, registering OR-backed object types at boot while preserving every legacy store and the filinq-specific settings store. The cutover to the shared store (Phase 2) is documented but deferred.

## Requirements

### Requirement: REQ-DSM-1 lib `useObjectStore` is the canonical OR-CRUD store

Filinq SHALL adopt `@conduction/nextcloud-vue`'s `useObjectStore`
...
- **THEN** the import SHALL resolve to a `useObjectStore` produced
  by `createObjectStore('filinq-objects')` at module load
...
`initializeStores()` SHALL register every OR-backed filinq
object type declared in `lib/Settings/filinq_register.json`
...
The filinq-specific `useSettingsStore` (`src/store/modules/settings.js`) SHALL remain in place, since it talks to `/apps/filinq/api/settings` and exposes `{ config, openRegisters, isAdmin }` used to gate admin-only UI.
...
- **AND** `useSettingsStore().config` SHALL contain the filinq
...
- **THEN** there SHALL be no Pinia store-id collision (the lib
  uses `'filinq-objects'`; the legacy stores use IDs
...
- **THEN** the design.md SHALL list the trigger conditions:
  (a) the filinq REST controller behind the feature delegates to
  OR,
  (b) a new feature requires lib sub-resource plugins (live
  updates, audit, files, relations) on a legacy store path,
  (c) a filinq manifest renderer ships and the manifest page
