# filinq-mcp-surface Specification (delta)

Extends Filinq's agent-facing surface with two approval-gated curated write tools
...
and `FilinqScannableServices` MUST include `SigningService::class`. The tool MUST be
...
Filinq MUST NOT implement a parallel grant or approval mechanism of its own.
...
- WHEN it chains `filinq.generateCorrespondence` and `filinq.startSigningRequest`
...
- GIVEN the registered tool `filinq.startSigningRequest`
...
- WHEN `filinq.recordConsentDecision` runs after gate approval
...
`filinq-mcp-adoption` and restated by `mcp-generation-tools` MUST hold across this
...
The signing **act** is never agent-performable. Filinq MUST NOT add `#[McpTool]` to
...
`filinq.startSigningRequest` (REQ-DDHAT-001): *initiating* a request, through the
