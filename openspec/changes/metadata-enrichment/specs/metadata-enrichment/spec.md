...
### REQ-META-001: Language Detection (Priority: Must)
...
- AND Dutch word count exceeds 5 and is higher than English word count
...
### REQ-META-002: Keyword Extraction (Priority: Must)
...
### REQ-META-003: Topic Classification (Priority: Must)
...
- GIVEN a document with 5 legal keyword matches and 3 financial keyword matches
...
### REQ-META-004: Document Type Standardization (Priority: Must)
...
- GIVEN documents with types "xls", "xlsx", or "excel"
- WHEN type standardization runs
...
### REQ-META-005: Date Normalization (Priority: Must)
...
- WHEN date normalization runs
- THEN the field is skipped gracefully (debug log recorded, no exception thrown)
...
### REQ-META-006: Event-Driven Enrichment (Priority: Must)
...
### REQ-META-007: API On-Demand Enrichment (Priority: Must)
...
- WHEN enrichment is triggered via the API
...
### REQ-META-008: Duplicated ObjectService Resolution (Priority: Must)

MetadataService carries its own private `getObjectService()` duplicating the pattern found in other services.
...
- AND consolidation to SettingsService::getObjectService() is recommended as a follow-up
...
### REQ-META-009: Event Listener Service Resolution (Priority: Must)
...
| META-074 | `\OC::$server` usage reduces testability (anti-pattern, known debt) | MUST | Bug |
...
### REQ-META-010: Text Content Extraction from Object Data (Priority: Must)
...
| ID | Requirement | Priority | Status |
|----|-------------|----------|--------|
| META-080 | Extract text from object fields in priority order: content → text → description | MUST | Implemented |
...
|-------|------|-------------|
| language | string\|null | ISO 639-1 language code (nl, en) or null if undetermined |
| keywords | array[string] | Top 10 keywords by frequency, stop-word-filtered |
| topic | string\|null | Topic category: legal, financial, medical, technical, or null |
| documentType | string | Standardized canonical document type |
| created | string (ISO 8601) | Normalized creation date |
| modified | string (ISO 8601) | Normalized modification date |
| date | string (ISO 8601) | Normalized generic date field |
| creationDate | string (ISO 8601) | Normalized creation date (alternate field name) |
| modificationDate | string (ISO 8601) | Normalized modification date (alternate field name) |

## API Endpoints

| Method | Endpoint | Description |
|--------|----------|-------------|
| POST | `/apps/docudesk/api/metadata/enrich` | Trigger metadata enrichment for an object |

### POST /api/metadata/enrich — Request

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| objectId | string | Yes | OpenRegister object identifier |
| register | string | Yes | OpenRegister register slug |
| schema | string | Yes | OpenRegister schema slug |
| objectData | object | No | Object data passed directly; skips extra lookup |

### POST /api/metadata/enrich — Response

| Field | Type | Description |
|-------|------|-------------|
| message | string | Human-readable result summary |
| enrichedFields | array[string] | List of field names that were enriched |
| object | object | Updated object data after enrichment |

## Standards & References

- **ISO 639-1**: Language codes (nl, en)
- **Dublin Core (ISO 15836)**: Metadata fields align with Dublin Core vocabulary
- **ISO 8601**: Date normalization target format
- **DCAT-AP**: EU open data metadata requirements
- **OWMS**: Dutch government metadata standard (Overheid.nl Web Metadata Standard)
