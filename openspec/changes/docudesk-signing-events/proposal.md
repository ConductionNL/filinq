# Proposal: docudesk-signing-events

## Why

DocuDesk is the fleet's document e-signature hub. Consumer fleet apps (e.g. shillinq) must be
able to delegate document signing to it when both apps are installed. The previously-shipped
delegation path used a phantom mechanism — `$registry->call('docudesk','createSigningRequest',…)`
— for which no such registry method exists. The call therefore always failed closed and the
request never reached DocuDesk: delegated signing was silently broken.
...
`DecisionRequestedListener` + `DecisionLifecycleService` emission). DocuDesk needs the same
...
DocuDesk already has `OCA\DocuDesk\Service\SigningService` with `createRequest()`, `sign()`,
...
1. **`OCA\DocuDesk\Event\DocumentSigningRequestedEvent`** — a consumer dispatches this to ask
   DocuDesk to raise a signing request for one of its objects. Immutable provenance + request
...
2. **`OCA\DocuDesk\Event\SigningConcludedEvent`** — DocuDesk dispatches this when a delegated
...
3. **`OCA\DocuDesk\EventListener\DocumentSigningRequestedListener`** (implements
...
- `docudesk-signing-events`: the cross-app event contract (request + conclusion events,
  listener, provenance persistence, terminal emission) through which fleet apps delegate
  document signing to DocuDesk.
...
- `openspec validate docudesk-signing-events --strict` exits 0.
- A consumer that dispatches `DocumentSigningRequestedEvent` (with a complete subject reference
  ...
  provenance.
- When that signing request reaches COMPLETED / DECLINED / CANCELLED / EXPIRED, DocuDesk
