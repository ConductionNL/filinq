...
Filinq's agent-facing MCP tool surface under ADR-063: the curated schema allowlist, the
...
### Requirement: Filinq declares a curated read-only MCP schema allowlist
Filinq MUST expose exactly eight schemas to MCP via `configuration.x-openregister-mcp` in
`lib/Settings/filinq_register.json` — `template`, `huisstijl`, `correspondence`,
...
`description`. No other Filinq schema may carry an `x-openregister-mcp` block.

#### Scenario: The eight allowlisted schemas expose derived read tools
- **WHEN** OpenRegister derives its tool surface from Filinq's register
- **THEN** exactly sixteen derived tools exist — `filinq.{schema}.search` and
  `filinq.{schema}.get` for each of the eight allowlisted schemas
- **AND** each tool reports `readOnlyHint: true` and `scope: read`

#### Scenario: A non-allowlisted schema exposes no tool
- **WHEN** an agent lists the Filinq tool surface
...
### Requirement: No Filinq schema exposes a derived write verb
The `create`, `update` and `delete` verbs MUST NOT be enabled on any Filinq schema.
...
#### Scenario: An agent cannot create, update or delete a Filinq object
- **WHEN** an agent enumerates Filinq's derived tools
...
- **THEN** no MCP tool exists to do so, and the agent MUST report that template authoring
  is a human action in the Filinq UI
...
- **WHEN** `filinq_register.json` is imported into OpenRegister
...
### Requirement: Filinq exposes one curated document-generation tool
...
`lib/Mcp/FilinqScannableServices.php` MUST implement OpenRegister's
`IMcpScannableServices` and MUST include `CorrespondenceService::class` in its returned
list. `generateCorrespondence` MUST remain Filinq's sole curated document-*generation*
...
- **WHEN** OpenRegister scans Filinq's `IMcpScannableServices` implementation
- **THEN** the tool `filinq.generateCorrespondence` is registered
...
Filinq MUST NOT add `#[McpTool]` to `SigningService`, `SigningVerificationService`,
...
- **WHEN** an agent enumerates Filinq's tool surface
...
- **THEN** `filinq.signingRequest.search` answers from `status`, `signatureLevel`,
...
- **THEN** `filinq.batchCorrespondenceJob.search` answers from `status`,
...
- **THEN** no MCP tool exposes `publicationProhibition`, and the agent MUST refer the user
  to the Filinq UI, where the access is authorised and audited
