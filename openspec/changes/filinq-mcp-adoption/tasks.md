# Tasks — filinq-mcp-adoption

## 1. Declare the dialect (config)

- [x] 1.1 Add `configuration.x-openregister-mcp` (`enabled: true`, `search` + `get`, `scope: read`, `readOnlyHint: true`, agent-facing `description` per verb) to `template`, `huisstijl`, `correspondence`, `generatedDocument` in `lib/Settings/filinq_register.json`.
...
- [x] 1.4 `python3 -m json.tool lib/Settings/filinq_register.json` — valid, still 18 schemas, no key dropped, indentation preserved.
...
- [x] 3.2 Assert the derived surface is exactly 16 tools (8 schemas × search+get) plus the curated tools, and that no tool name ends in `.create` / `.update` / `.delete`. **Measured live:** 20 `filinq.*` tools in `ToolOversightController::toolCatalog()` — 16 derived + 4 curated (`generateCorrespondence`, `readDocument`, `editDocument`, `convertDocumentToPdf`); zero write verbs.
...
- Eight schemas expose `search` + `get`; no Filinq schema exposes a derived write verb.
