...
Filinq today has **no MCP surface at all** — no provider class, no dialect, no
...
obvious things a user would ask an assistant. This change is Filinq's greenfield
adoption of ADR-063.

Filinq is also the fleet's highest-consequence app for getting the *boundary* right: it
...
- **Declare `x-openregister-mcp` on 8 curated schemas** in `lib/Settings/filinq_register.json`,
...
- **No derived `create` / `update` / `delete` verb is enabled on any Filinq schema.**
...
- **Add the curated generation `#[McpTool]`**: `CorrespondenceService::generate()` becomes
  `filinq.generateCorrespondence` — a genuinely non-CRUD, multi-step action (fetch
...
- **Add `lib/Mcp/FilinqScannableServices.php`** implementing OpenRegister's
...
### New Capabilities
- `filinq-mcp-surface` — Filinq's agent-facing tool surface: the curated schema
...
_None._ No existing Filinq requirement changes behaviour: the dialect is additive
...
- **Config:** `lib/Settings/filinq_register.json` — 8 schemas gain a
...
- **Code:** `lib/Service/CorrespondenceService.php` (one attribute + `use` import),
  new `lib/Mcp/FilinqScannableServices.php`.
...
- **Consumers:** Hermiq is the sole agent consumer; it picks the tools up from OR's
  registry (JSON-RPC `/api/mcp` + the chat facade). No Filinq controller, route or
  frontend changes.
