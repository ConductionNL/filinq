...
Filinq canonical-feature **mailbox-integration cluster** collects the
...
Filinq already owns half the machinery, verified at HEAD:
...
- **Threading metadata**: Filinq parses `In-Reply-To`/`References` from
...
- **IMAP boundary (decided and documented)**: mailbox fetching does NOT
  enter Filinq. Source-system connectivity and mailbox credentials belong
  ...
  `.eml` files into the watched folder". Filinq stores no mailbox
...
- Admin settings: `filinq.email_ingestion.inbox_folders` (folder →
  dossier mapping), `filinq.email_ingestion.files_per_tick`.
...
- Boundary: optional IMAP fetch = OpenConnector flow writing into the
  watched folder (documented contract; zero Filinq mailbox code).
