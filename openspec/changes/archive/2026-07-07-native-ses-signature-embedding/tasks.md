...
- [x] T-1.1 Implement, in `NativeSigningProvider`, production of a signed PDF that embeds a
...
- [~] T-1.2 Route non-PDF or encrypted inputs through the existing `PdfConversionService` /
  ...
  - **DEVIATION (faithful-partial):** the writer embeds the marker into the supplied document bytes
    and any read/write failure throws (honest-completion gate — the request never falsely
    completes). The explicit pre-embed `PdfConversionService` conversion + `DocumentValidationService`
    `pdf-encrypted` short-circuit was NOT wired: for a well-formed PDF the artifact is produced and
    verifies; for a non-writeable input the failure still surfaces loudly at store time rather than
    at a dedicated convert/validate step. Wiring the conversion/validation pre-step is a follow-up.

## [docudesk] Wire the provider into completion

### T-2. Produce + store the artifact on the completing signature (M)

- [x] T-2.1 In `SigningService`, on the signature that transitions a request to COMPLETED, resolve
...
- [x] T-2.2 In `updateRequestStatus()`, set `signedDocumentRef` to the stored artifact reference
...
- [x] T-3.1 Ensure that when the active provider cannot produce an artifact (native writer
...
- [x] T-4.1 In `docs/GOVERNMENT-FEATURES.md`, adjust F-13 so it presents the signing workflow +
  audit trail as available and signature embedding as in progress (reference `#304`) rather than a
  bare "Beschikbaar".
- [x] T-4.2 In `docs/features.json`, adjust the signing summary so it no longer states signature
...
- [x] T-5.1 PHPUnit: a native SES request whose signers all sign produces a stored artifact that
...
- [x] T-5.2 `openspec validate native-ses-signature-embedding --strict` exits 0.
- [x] T-5.3 `php -l` passes on every changed PHP file; self-check the key hydra gates (SPDX headers,
