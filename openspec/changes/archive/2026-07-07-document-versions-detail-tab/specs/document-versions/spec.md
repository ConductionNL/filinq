...
Add the `Versies` tab declared in ADR-001 to the `Document detail` surface: list, open, restore,
and compare the Nextcloud file versions of a document, consuming the platform's `files_versions`
capability. No new storage or versioning engine is introduced; DocuDesk reads Nextcloud's own
version history and reuses the existing `DocumentComparisonService` for the compare action.
