...
@e2e exclude Backend extraction service + REST API + NC-event contract — no dedicated DocuDesk browser surface (the scan-en-herken UI lives in the consumer app, shillinq). Deterministic heuristic extractors, confidence scoring, event payload shape, graceful AI degradation, and the correction-feedback store are all covered by PHPUnit and Newman (API) tests.
