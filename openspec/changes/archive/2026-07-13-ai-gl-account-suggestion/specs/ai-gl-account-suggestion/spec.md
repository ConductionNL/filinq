...
@e2e exclude Backend ranking service + REST API + NC-event contract — no dedicated DocuDesk browser surface (the booking UI lives in the consumer app, shillinq, per the same split as financial-document-field-extraction). Deterministic ranking, cold-start fallback, graceful AI degradation, the correction-feedback hook, and the event payload shape are all covered by PHPUnit.
