# adopt-apphost Specification (delta)
...
Move DocuDesk's `/api/health` and `/api/metrics` endpoints onto OpenRegister's AppHost
observability engine (ADR-006 / ADR-040): the endpoints are served by thin DocuDesk subclasses of
the engine-owned generic controllers, driven by a declarative `observability` block in
`src/manifest.json`. The bespoke health checks, metric lines, and `MetricsCollector` are removed.
Endpoint URLs and the admin/public auth posture are preserved (with health correctly made public).
