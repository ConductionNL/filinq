...
Record the boundary between docudesk's **WOO publication-disclosure consent** workflow
and OpenRegister's canonical **GDPR data-subject-rights** capability
(`DataSubjectRequestService` + `DataSubjectDeadline`, `/api/gdpr/*`), and the rule that
the WOO publication objection period MUST NOT be delegated to OR's EU GDPR art-12(3)
deadline helper.
