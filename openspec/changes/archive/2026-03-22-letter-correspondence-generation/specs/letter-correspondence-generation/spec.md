...
The system SHALL support multiple output formats for correspondence: PDF (default), DOCX (via LibreOffice server-side conversion), and HTML (for preview). The format SHALL be selectable per request via the `options.format` field accepting values `pdf`, `docx`, or `html`.
