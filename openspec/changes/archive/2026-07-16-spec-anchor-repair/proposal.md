...
on a mass scale — **864 in docudesk** on base `origin/development`, out of
949 total `@spec` tags (91%). A broken anchor is code *claiming* to
...
- **484 anchors repointed** across 97 files
  (83 to an exact requirement heading, 401 downgraded to
  capability-level file granularity).
- **380 anchors left untouched** and filed for human review
  (`residual-dangling.md` + umbrella issue) — never guessed.
- **gate-46 (`spec-anchor-existence`): 864 → 380 broken.**
- The diff is **`@spec`-comment-lines only**: 968 changed lines, **0**
...
- Gate-46 noise drops by 56%, so a *newly* broken anchor in a PR is now
  visible instead of drowning in 864 pre-existing failures.
