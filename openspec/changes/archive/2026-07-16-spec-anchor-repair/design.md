# Design — spec-anchor-repair (docudesk)

## Category breakdown (base `origin/development`)

864 broken `@spec` anchors, classified by recoverability:

| Category | Count | Disposition |
|---|---|---|
| **(c) archived change → canonical spec recoverable** | **484** | **auto-repointed** |
| &nbsp;&nbsp;↳ exact requirement-heading match | 83 | `specs/<cap>/spec.md#requirement-<slug>` |
| &nbsp;&nbsp;↳ capability proven, heading not matched | 401 | `specs/<cap>/spec.md` (file-level) |
| **(d)**  change uses non-annotate tasks.md (no `task-N: cap#REQ` line) — needs spec-delta read | 163 | flag |
| **(d)**  archived change dir not located | 116 | flag |
| **(d)**  non-tasks.md ref (decimal task / design.md / proposal.md / re-headed specs anchor) | 95 | flag |
| **(d)**  no-fragment tasks.md ref, slug not a canonical capability | 3 | flag |
| **(c)**  capability archived/deleted — requirement genuinely gone | 3 | flag |
| **Total flagged for human triage** | **380** | `residual-dangling.md` |

Auto-repointed **56%**; the residual 380 stay broken *on purpose*
...
4. **Post-condition gate** — every proposed target is re-checked with gate-46
   ...
   and the anchor stays dangling. **Observed on docudesk: 0 rejects.**
...
484 anchors is infeasible and more error-prone, and the safety property
...
The residual 380 anchors are **not** force-removed or re-pointed on a
