...
# Folder Analysis Anonymization — Delta for Output Folder Layout

This delta extends the existing `folder-analysis-anonymization` capability so folder-level orchestration (the analyse-and-anonymise-a-folder flow) writes redacted outputs to the same `<source-folder>/<subfolder>/` layout defined by the `batch-anonymization` delta. Behaviour is consistent across the two folder-driven entry points; configuration is shared.
