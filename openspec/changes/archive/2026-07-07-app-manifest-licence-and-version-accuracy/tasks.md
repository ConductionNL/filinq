...
- [x] T-1.1 In `appinfo/info.xml`, change `<licence>agpl</licence>` to
...
- [x] T-2.1 In `docs/GOVERNMENT-FEATURES.md`, change the header "Licentie: AGPL (vrije open
  source)" to "Licentie: EUPL-1.2 (vrije open source)".
- [x] T-2.2 In the same file, change technical requirement T-02 toelichting "AGPL, GitHub" to
  "EUPL-1.2, Codeberg".
  - **Acceptance:** no "AGPL" token remains in `docs/GOVERNMENT-FEATURES.md`.

### T-3. Align the README requirements table (S)

- [x] T-3.1 In `README.md`, update the "Requirements" table to match `appinfo/info.xml`:
...
- [x] T-4.1 `grep -rn "agpl\|AGPL" appinfo/ docs/ README.md composer.json publiccode.yml` returns
...
- [x] T-4.2 `openspec validate app-manifest-licence-and-version-accuracy --strict` exits 0.
- [x] T-4.3 No user-facing English l10n key is added or changed (the l10n parity gate stays green).
