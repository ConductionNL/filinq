...
Converts an existing PDF or freshly rendered HTML into a genuinely PDF/A-3b
compliant document — XMP metadata identifying the conformance level, an
embedded ICC output intent, embedded fonts, and embedded file attachments
(the feature that actually distinguishes PDF/A-3 from PDF/A-1/A-2) — for
MDTO/archival compliance. Consumed by procest's beschikking/archival
pipeline and OpenRegister's TMLO/MDTO e-depot SIP builder. See design.md for
the converter decision (mPDF's native FPDI import, no new dependency) and
the validation-scope boundary (container-level markers, not full
veraPDF-grade content validation).
