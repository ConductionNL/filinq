...
- [x] 4.5 Quality gates (re-confirmed 2026-07-24 after a second merge of origin/development, baseline = a fresh worktree at origin/development @ 6b4f69d5 with the same vendor/ copied in): full PHPUnit suite green — **1053/1053** (0 failures/errors, 1 pre-existing skip, 2 pre-existing deprecations) vs baseline **998/998** (same skip/deprecation counts) — i.e. +55 tests (44 from this stack + 11 picked up from sibling waves merged in), all passing, zero regressions. PHPCS: **0 errors** on the full tree (183 warnings) vs baseline **0 errors / 196 warnings** — fewer warnings than baseline, zero new. PHPStan: **2 errors**, both pre-existing and unrelated (`CustomDictionaryMatchService.php:219`, `CustomDictionaryService.php:402` — `custom-dictionary-recognition` wave), byte-identical to the baseline's 2 errors at the same lines. Psalm: **1 error**, pre-existing and unrelated (`AnonymizationService.php` `GdprEntity` undefined class), identical to baseline's 1 error. `openspec validate --all`: **111 passed, 0 failed** (the previously-known 3 pre-existing failures — `beta-surface-alignment`, `document-preview`, `signing-via-or-approval-with-provider-plugins` — were repaired by the wave-3 merge picked up in this pass); `openspec validate signing-trust-rebuild --strict` → **valid**.
