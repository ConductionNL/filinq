...
- [x] 4.5 Quality gates (re-confirmed 2026-07-23 after merging origin/development, baseline = origin/development @ 61746af0 with the same vendor/ copied in): full PHPUnit suite green — **1042/1042** (0 failures/errors, 1 pre-existing skip, 2 pre-existing deprecations) vs baseline **998/998** (same skip/deprecation counts) — i.e. +44 tests, all passing, zero regressions. PHPCS: **0 errors** on the full tree (183 warnings, all pre-existing `@spec`-tag style warnings on untouched files) vs baseline **0 errors / 196 warnings** — fewer warnings than baseline, zero new. PHPStan: **2 errors**, both pre-existing and unrelated (`CustomDictionaryMatchService.php:219`, `CustomDictionaryService.php:402` — `custom-dictionary-recognition` wave), byte-identical to the baseline's 2 errors at the same lines. Psalm: **1 error**, pre-existing and unrelated (`AnonymizationService.php` `GdprEntity` undefined class), identical to baseline's 1 error. `openspec validate --all`: **108 passed, 3 failed** — the 3 failures are exactly the known pre-existing set (`beta-surface-alignment`, `document-preview`, `signing-via-or-approval-with-provider-plugins`); `openspec validate signing-trust-rebuild --strict` → **valid**.

**Overall**: the security core (D1–D7 / REQ-DDSTR-001..007, 009, 010) is implemented and unit-tested; this change is left ACTIVE (not archived) because 3.2 (live immutability proof), 4.2 (Playwright e2e), and 4.4 (docs + issue closure) are genuinely unfinished and require a live Postgres/Nextcloud verification pass this session did not perform (the shared dev instance on :8080 is off-limits for deploying in-progress branch work per standing project rules).
