...
to a uuid-only `ObjectEntity` stub, retrieval scans every `docudesk.signing.*`
...
mapper/query layer) instead of fetching all `docudesk.signing.*` entries
...
attempt to modify or remove a `docudesk.signing.*` audit entry MUST be
...
- GIVEN an existing `docudesk.signing.SIGNED` audit entry in OR
