...
- [ ] 1.1 Add the `conformanceReport` schema (`fileId` idempotency key, `flavour`, `compliant`, `failedRuleCount`, `failedRules`, `fontsNotEmbedded`, `validatorVersion`, `validatedAt`, `trigger`) to `lib/Settings/filinq_register.json` — additive, union-merge only; references and font names only, never content (REQ-DDVPV-004)
...
- [ ] 2.1 New `VeraPdfService`: probe (`isAvailable()`, version) via `filinq.verapdf.binary_path`/`enabled`, CLI invocation with `filinq.verapdf.max_seconds` wall-clock budget, JSON result parsing pinned to the read fields, typed `VeraPdfException`; a validator error never yields `compliant` (REQ-DDVPV-001, REQ-DDVPV-002)
- [ ] 2.2 Font-embedding results + remediation-guidance classification (Filinq-rendered → regenerate; imported/uploaded → re-convert from source, honest retroactive limitation) (REQ-DDVPV-003)
...
- [ ] 2.5 `Pdfa3ConversionService` post-output verification: `X-Docudesk-Pdfa3-Verified` header, `trigger: "conversion"` report, report-mode default, `filinq.pdfa3.strict_verify` → `Pdfa3ConversionException` reason `output_validation_failed`; marker guard untouched (REQ-DDVPV-006)
