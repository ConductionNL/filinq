...
Defines a policy layer that pre-empts the publication-clearance workflow at detection time. Two policy surfaces are introduced:
...
- The `publicationConsent` schema (in the consent register, `lib/Settings/docudesk_register.json`) tracks per-(document, entity) workflow state for WOO compliance: notification → 28-day objection → decision. Today every record is implicitly per-document; this change introduces an explicit `scope` discriminator (specified in the `consent-management` delta).
- There is no entity-level representation of "always anonymize" rules (court orders, threat assessments, minor protections, categorical AVG exemptions), nor of "standing consent" rules (public officials' standing publication consent, signed opt-in by individuals or organizations).
- Detection-time matching has no policy layer; there is no automated path that creates `publicationConsent` records at all today (per canonical `consent-management` REQ-CONS-07: no API endpoint or automated trigger exists). Once a publication-prep flow lands and starts calling `createConsentRequest()`, the policy layer must be consulted at that entry point.
- OpenRegister's `ValidateObject::extractObjectConfigurationHandling` already supports `items.oneOf` + `$ref` polymorphic references with `objectConfiguration.handling: "related-object"`. This is the mechanism that makes the new `policyMatch` field on `publicationConsent` type-safe.
