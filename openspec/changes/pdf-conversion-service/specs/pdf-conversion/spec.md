...
Defines a generic file-to-PDF conversion service that takes an arbitrary supported input file and produces a PDF/A-3b output. Implements a cascade of conversion backends (Office app integrations → LibreOffice headless → PhpWord+mPDF → mPDF directly → OR-EML-extractor) with per-backend availability checks, tenant-configurable backend order, and structured failure reporting. Anonymisation is the first consumer; other consumers may follow.
