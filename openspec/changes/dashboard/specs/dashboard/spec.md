...
- GIVEN DashboardController::page() accepts `?string $getParameter`
- WHEN the method is called with or without this parameter
- THEN the parameter has no effect on the response
- AND this is documented as dead code for future cleanup
...
- THEN the mapping is:
  ...
  - `consent_given` → label "Approved", color green (success)
  - `objection_received` → label "Objected", color red (error)
  - `no_response` → label "No Response", color orange (warning)
  - `anonymized` → label "Anonymized", color blue
...
- **Consent store**: `consentStore.consents` (recent consent records, each a `PublicationConsent` object with `entityText` and `consentStatus`)

No OpenRegister schema objects are defined or modified by this feature. All data is owned by the `consent-management` capability.

## API Endpoints

| Method | Endpoint | Description |
|--------|----------|-------------|
| GET | `/` | Render DocuDesk main page (TemplateResponse / SPA shell) |

## Dependencies

- **Pinia navigationStore** (`src/store/modules/navigation.ts`): View switching and active nav state
- **Pinia consentStore**: Consent data and statistics consumed by DashboardIndex.vue
- **Pinia anonymizationStore**: Quick anonymization pipeline consumed by embedded AnonymizationWidget
- **Nextcloud IWidget / IIconWidget**: Dashboard widget registration API
- **Nextcloud NcAppNavigation**: Navigation component framework used by MainMenu.vue
- **ADR-003**: NL Design System tokens via Nextcloud CSS variables for color-coded stat cards
- **ADR-012**: `@conduction/nextcloud-vue` components (CnDashboardPage) for page layout
...
| `src/views/Views.vue` | Conditional view rendering driven by navigationStore |
| `src/navigation/MainMenu.vue` | Three-item sidebar navigation |
...
- **ADR-001**: All data via OpenRegister — no custom database tables
- **ADR-003**: NL Design System tokens — no hardcoded colors
- **ADR-012**: Use `@conduction/nextcloud-vue` components
