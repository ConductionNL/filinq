...
- [x] 1.1 `SettingsService`: add `docudesk.grondslagen.entity_type_bases` — load via `json_decode(getValueString(...))` (default `{}`), save via `setValueString(json_encode(...))`; value is `{ "<TYPE>": ["<base-slug>", …] }`.
...
- [x] 2.1 Define a curated entity-type constant in DocuDesk, seeded from the backend's known emitted types (GLiNER `entity_mapping` targets + enabled pattern recognizers' types); document its provenance and verify the identifier strings against a live `/analyze` response.
...
- [x] 3.1 Add a grondslag-mapping section to the DocuDesk settings page: one row per curated entity type (from 2.1), each with a multi-select of `base` records (from 2.2), defaulting to a single selection.
...
- [x] 6.2 Update DocuDesk settings/help docs to describe the grondslag-per-entity-type mapping.
...
- [ ] 7.3 DocuDesk: swap the curated-list seam (2.3) for the live backend list, cached with fallback to the curated list.
