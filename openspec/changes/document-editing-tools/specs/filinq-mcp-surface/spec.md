...
Delta: extends Filinq's curated MCP surface with two tools that act on
documents which already exist. The schema allowlist declared by
`filinq-mcp-adoption` is unchanged — this change adds **no** schema and enables
...
Filinq MUST expose exactly two further curated `#[McpTool]` methods via the
`IMcpScannableServices` path: `filinq.convertDocument`, which MUST produce a new
file and MUST NOT mutate its input; and `filinq.editDocument`, which by default
...
- **WHEN** an agent lists Filinq's tool surface
- **THEN** `filinq.convertDocument` and `filinq.editDocument` MUST be present
  in addition to `filinq.generateCorrespondence`
...
`filinq.convertDocument` MUST declare `scope: "create"`, `readOnlyHint: false`,
`destructiveHint: false`. `filinq.editDocument` MUST declare `scope: "update"`,
...
`filinq-mcp-adoption` are extended, never relaxed.
