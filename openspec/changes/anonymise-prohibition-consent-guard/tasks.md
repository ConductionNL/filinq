...
- [x] 1.2 Read the threshold from app config `filinq.prohibition.high_confidence_threshold` (default 0.85) at request time. Single reader used by both analysis and the guard.
...
- [x] 3.1 Add `PATCH /apps/filinq/api/anonymization/relations/{id}` (route + `AnonymizationController` method). Accepts `skipAnonymization`, optional `bases`, optional `force`.
...
- [x] 4.3 On skip-toggle, call the Filinq skip endpoint (not OR's PATCH) immediately; catch its 422 into an error dialog showing the blocked occurrence (canonical name + rule name), and offer a `force` retry only when `absolute` is false.
