...
No OpenRegister schema change: `EntityRelation` is an internal OR database entity, so persisting a Filinq-policy flag on it would require an OR migration and leak a Filinq concept onto a generic table. Instead the matcher runs at analysis (for the standing-consent auto-skip + the frontend hint) and again at the guard (a cheap in-memory pass over the file's entities).
...
- **NEW:** A Filinq-owned per-relation skip-decision endpoint (`PATCH /apps/filinq/api/anonymization/relations/{id}`) that the review UI calls on skip-toggle, in place of PATCHing OpenRegister's `/api/entity-relations/{id}` directly. Setting `skipAnonymization = true` on a prohibition-matched relation is guarded per occurrence, at decision time:
  ...
  Including an entity (or a non-skip decision such as `bases`) is always allowed. Allowed decisions forward to OR's `updateDecisionMetadata`. The guard lives in Filinq because the prohibition policy is Filinq-owned; OR's generic endpoint cannot evaluate it without coupling.
...
- **NEW:** The threshold is `filinq.prohibition.high_confidence_threshold` (app config, default `0.85`), read at request time so runtime changes propagate without restart. The same threshold is used at analysis (`highConfidence`) and at the guard.
...
Accountability: OpenRegister's `EntityRelation` audit-trail already records the actor + timestamp of any `skip_anonymization` flip, so a `force`d sub-threshold skip is auditable without a bespoke Filinq audit store.
...
- **Code (filinq):**
  ...
  - Frontend: the review store calls the Filinq skip endpoint on toggle (not OR's PATCH), plus a new 422 error dialog component.
- **API contract:** new Filinq skip endpoint (`PATCH /apps/filinq/api/anonymization/relations/{id}`, `force` optional, 422 with `prohibitionMatch` + `threshold`); extract response gains `prohibitionMatch` (additive). Callers with no prohibition records see no change.
...
- **Soft** — `filinq:entity-publication-policies` provides the `publicationProhibition` / `standingPublicationConsent` schemas the matcher reads. The guard is a no-op until such records exist.
