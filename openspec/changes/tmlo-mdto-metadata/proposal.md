---
kind: code
tracking_issue: https://github.com/ConductionNL/filinq/issues/237
...
Evidence: tracking issue ConductionNL/filinq#237; intelligence DB
...
per-object fields (`tmlo` vs `retention`). Per ADR-022 Filinq consumes
these surfaces and supplies only what is genuinely Filinq domain
...
2. **Auto-prefill from Filinq context**: Filinq derives supplement
   ...
   Filinq-unique capability: the app knows *why* a record's use is
...
5. **e-Depot handoff**: transfer rides OR's `EdepotTransferService`
   (SIP + transports + profiles configured in OR settings); Filinq ships
...
- `tmlo-mdto-metadata`: MDTO (TMLO-successor) archival metadata for
  Filinq records — supplement schema for the informatieobject attributes
  OpenRegister does not model, Filinq-context auto-prefill (including
...
- [x] Project: `filinq` — `mdtoSupplement` schema + seeds,
...
- e-depot transport/protocol implementations (OR owns transports/profiles);
  no Filinq HTTP client to any e-depot.
- A generic MDTO editor for arbitrary OR registers (Filinq documents and
  dossiers only).
...
- A document record shows an MDTO panel with core fields (read-only, from
  `retention`) and supplement fields, prefilled from Filinq context where
  derivable.
