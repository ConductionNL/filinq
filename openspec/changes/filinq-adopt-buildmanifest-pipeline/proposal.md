# Proposal: filinq-adopt-buildmanifest-pipeline
...
Filinq currently ships exactly the forbidden bespoke copy. `src/main.js`
defines, inline:
...
duplication this ADR was written to retire. Filinq's own comment in
`applyMenuLayout`'s docblock (`src/main.js:236`) even says *"Filinq ships a
...
Filinq's pre-manifest navigation, superseded by the manifest-driven
...
- Any actual navigation-IA change (populating `menu-layout.json`'s
  relocations/sections/removals/settingsSection) — Filinq's own note in
