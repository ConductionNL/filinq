...
that require document handling against case systems. Today Filinq can only
...
which disqualifies Filinq from every one of these tenders.
...
`synced-from-tab`). What is missing is the **Filinq-side contract**: the
...
- New `bridge` register in `lib/Settings/filinq_register.json` with two
  schemas:
...
- A documented, versioned **bridge contract** between OpenConnector and
  Filinq: OpenConnector writes/updates `externalDocument` objects (inbound
  leg) and picks up objects in status `ready_for_writeback` (outbound leg);
  Filinq never talks ZGW/StUF itself.
...
- Dossier pick-up: external documents can be attached to a Filinq dossier and
...
- UI: a source badge ("Zaaksysteem: <vendor>") on externally-sourced documents
  in MyDocuments/document detail, and a bridge status panel in Filinq admin
...
- `zgw-document-bridge`: Filinq-side contract for processing documents
...
- `lib/Settings/filinq_register.json`: new `bridge` register + 2 schemas +
