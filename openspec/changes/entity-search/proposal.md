...
question Filinq cannot answer today, and it is the question both the market
...
- The repo inventory lists **no cross-document search** as a notable absence:
  Filinq detects entities per file and reviews them per batch, but an
...
The data already exists — Filinq just has no window onto it. Verified at
...
do NOT live in Filinq's `anonymizationLink` objects (those only pair source
and anonymized file) nor in file metadata. Filinq-side, `anonymizationLink`
...
- **Permission gate**: access restricted to admin-configured Nextcloud groups
  (`filinq.entity_search.allowed_groups`, empty default = admins only,
...
- **Processing log** (AVG Art. 30): every search and every entity-detail view
  ...
  reference). The activity is declared as a `filinq-entity-search`
  `x-openregister-processing` annotation so it appears in the platform
  verwerkingsregister alongside Filinq's existing four activities.
...
- `lib/Settings/filinq_register.json`: new `entitySearchLog` schema in the
  `document` register with the `filinq-entity-search`
...
- Admin settings: `filinq.entity_search.allowed_groups`.
- Consumes (unchanged): OR entity catalogue + `RiskLevelService`
  (presence-gated — Filinq loads without OR), `anonymizationLink` lookups,
