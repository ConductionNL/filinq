...
- [x] T10 — Quality gates (re-confirmed 2026-07-23 after merging origin/development, baseline = origin/development @ 61746af0 with the same vendor/ copied in): `php -l` clean; PHPCS **0 errors** fleet-wide (183 warnings vs baseline 196 — fewer, zero new); PHPStan **2 pre-existing unrelated errors** identical to baseline; Psalm **1 pre-existing unrelated error** identical to baseline; the full unit suite is green — **1042/1042** (up from the 998 baseline of origin/development, +44, zero regressions); `openspec validate --type change portal-signing-surface --strict` → **valid**. Hydra gate script itself was not run (not present/invoked in this workflow); route-auth/route-reachability were verified for the new `PortalSigningReceiverController` routes (owned by `portal-signing-actions`, not re-verified here).
