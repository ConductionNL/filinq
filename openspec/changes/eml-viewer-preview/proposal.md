...
The natural fix for (2) is a **server-side render**, not a browser-side EML parser. Rendering on the server means any client — the Filinq viewer, a direct link, a future mobile client, another app — consumes one endpoint instead of re-implementing MIME parsing, encoding handling and inline-image resolution in JavaScript. It also lets us reuse the exact rendering that `eml-pdf-assembly` already ships: the original preview is simply the anonymise-assembly pipeline run with an **empty entity set** (nothing to redact), so there is no second "render an email" code path to maintain or keep consistent.
