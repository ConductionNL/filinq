# Design — docudesk-mcp-adoption

Context: ADR-063 (MCP as Platform Abstraction, hydra #102) + the fleet MCP wave,
2026-07-13. DocuDesk at HEAD `6344a616` has **no** MCP provider, **no** dialect and **no**
...
schema root. Every allowlisted DocuDesk schema already has a `configuration` object (with
...
| 6 | `signingRequest` | search, get | `status`, `signatureLevel`, `provider`, `initiatorUserId` | **"Has that contract been signed yet?"** — the single highest-value DocuDesk question. Process metadata only: no signature material, no signer emails, no IPs (those live on the excluded `signerRecord`). |
...
DocuDesk processes three categories of protected material. The allowlist is drawn so that
...
3. **Re-identification risk** — `anonymizationLink` is the join table between a redacted
   document and its original. DocuDesk's entire anonymisation feature exists to break that
...
- `python3 -m json.tool lib/Settings/docudesk_register.json` after every edit; schema count
...
- Scoped `phpcs` clean on `lib/Service/CorrespondenceService.php` and
  `lib/Mcp/DocudeskScannableServices.php`; zero new PHPUnit failures against a
...
4. **Field-level projection is missing from the dialect.** `x-openregister-mcp` is
   ...
   Several of DocuDesk's exclusions (and hrmq's `Employee`) would become safe, high-value
