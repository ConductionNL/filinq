# Tasks: docudesk-nl-locale-parity

All tasks are `[docudesk]`. Estimates: S = half-day, M = 1-2 days.

## [docudesk] Gate: enforce en/nl key parity
...
## [docudesk] Fix Dutch-as-key literals
...
- [ ] C-1.6 `src/components/DdEntityCard.vue:68` — same `'Grondslagen'` →
  English key rename.
  - **Acceptance:** `grep -rn "t('docudesk', '[A-Za-z]*[Gg]rondslag" src/`
...
- [ ] D-1.1 Re-run the repo-wide scan for any other Dutch word used as a
  `t('docudesk', ...)` literal key (spot-checked for "Grondslag" only in
