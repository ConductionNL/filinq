# Proposal: docudesk-nl-locale-parity

kind: code

## Why

DocuDesk's own `l10n/en.json` (the source-of-truth per ADR-007) currently
...
DocuDesk's primary government-user language (ADR-007: "All Conduction
...
`l10n-check [docudesk]: scanned 94 files, 596 distinct literal keys used, 682
...
`t('docudesk', ...)` key/msgid, not just as the eventual translated value:
...
- Rename the six Dutch-keyed literals above to English keys (e.g.
  `t('docudesk', 'Bases catalogue could not be loaded — falling back to the
  seeded list.')`, `t('docudesk', 'Legal basis (bases)')`, `t('docudesk',
...
- No `t('docudesk', ...)` call site anywhere in `src/` uses a Dutch word as
