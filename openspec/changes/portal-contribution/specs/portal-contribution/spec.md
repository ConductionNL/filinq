...
DocuDesk contributes `data-subject` and `signer` read surfaces to **portaliq**,
...
dependency-free provider class (`OCA\DocuDesk\Portal\PortalContributionProvider`,
...
The app MUST ship `OCA\DocuDesk\Portal\PortalContributionProvider` as a plain
...
- @e2e exclude backend-only contract class with no DocuDesk UI surface; the portal renders inside portaliq — covered by PHPUnit (tests/unit/Portal/PortalContributionProviderTest.php::testProviderIsPlainAndDependencyFree)
...
- @e2e exclude backend-only contract methods with no DocuDesk UI surface — covered by PHPUnit (testAudiencesOnBothContractVersions, testUnknownAudienceYieldsNull)
...
`DocuDesk` with exactly one read collection `subjectConsents` over register
...
- THEN it returns a manifest labelled `DocuDesk` with a single `subjectConsents` collection over `consent`/`publicationConsent`
...
- @e2e exclude manifest is consumed and rendered by portaliq, not by any DocuDesk UI — covered by PHPUnit (testDataSubjectConsentCollectionIsScopedAndProjected)
...
`DocuDesk` with two read collections over register `signing`:
...
- WHEN `getContribution($subject)` is called
- THEN it returns a manifest labelled `DocuDesk` with `signerRecords` (scoped `email` / claim `signerEmail`) and `signerSigningRequests`
...
- @e2e exclude manifest + via-join are consumed and enforced by portaliq's reader, not by any DocuDesk UI — covered by PHPUnit (testSignerRecordCollectionIsScopedAndProjected, testSignerSigningRequestsUsesViaJoin)
...
in `lib/Settings/docudesk_register.json`. A register drift (renamed scope
...
- GIVEN the shipped `docudesk_register.json` and the manifest for each served audience
...
are deferred to a follow-up (tracking issue Conduction/docudesk#160).
...
- @e2e exclude backend-only manifest shape with no DocuDesk UI surface — covered by PHPUnit (testNoActionsShipThisWave)
