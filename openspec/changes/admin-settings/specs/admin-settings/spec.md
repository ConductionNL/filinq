...
### REQ-SET-01: Nextcloud Admin Panel Integration
...
### REQ-SET-03: Auto-Initialization on Boot
...
Administrators can configure the publication objection period per WOO requirements, with validation ensuring awareness of the minimum 4-week objection period.

#### Scenario: Adjust objection period to 42 days
- **GIVEN** an administrator opens the DocuDesk admin settings
- **WHEN** they change the objection period to 42 days
- **AND** click Save All Settings
- **THEN** `publication_objection_period_days` is updated to "42"
...
| SET-032 | Display descriptive text referencing WOO minimum 4-week requirement (art. 4.4) | MUST | Implemented |

### REQ-SET-05: Metadata Enrichment Feature Toggles
...
### REQ-SET-07: SettingsService Public Helper Methods
...
- **AND** then checks `isOpenRegisterEnabled()` for the current user
...
### REQ-SET-08: App Metadata and Compatibility
...
### REQ-SET-10: Configuration File Resolution and Validation
...
### REQ-SET-11: TypeError Catch Fallback for OpenRegister

Settings retrieval gracefully handles `TypeError`s from OpenRegister internals to prevent crashes.
