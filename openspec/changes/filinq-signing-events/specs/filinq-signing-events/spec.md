# filinq-signing-events Specification (delta)
...
signing to Filinq when both apps are installed, replacing the broken
`$registry->call('filinq','createSigningRequest',…)` delegation path. Consumers dispatch a
`DocumentSigningRequestedEvent` to raise a signing request and listen for a `SigningConcludedEvent`
to consume the outcome. Filinq builds its side of the contract entirely on top of the existing
`OCA\Filinq\Service\SigningService` — no new signing engine, no new state machine, no schema
change. Mirrors decidesk's `decidesk-decision-events`.

## Cross-app contract — verbatim FQCNs and payloads

Consumers MUST reference these exact fully-qualified class names (autoloaded whenever Filinq is
installed):

- **`OCA\Filinq\Event\DocumentSigningRequestedEvent`** — request fields (immutable getters):
  ...
  (string). Result slot (set by Filinq during synchronous dispatch):
...
- **`OCA\Filinq\Event\SigningConcludedEvent`** — immutable fields: provenance
...
The listener is **`OCA\Filinq\EventListener\DocumentSigningRequestedListener`**.
...
`OCA\Filinq\Event\DocumentSigningRequestedEvent` extending `OCP\EventDispatcher\Event`,
available whenever Filinq is installed, that a consumer app dispatches to ask Filinq to raise
...
`setHandled(bool)` / `isHandled(): bool` — that Filinq's listener writes so the dispatching
...
- THEN after dispatch the event's `getSigningRequestId()` returns the id of the signing request
  Filinq created and `isHandled()` is true
...
The system SHALL provide an autoloaded public event class `OCA\Filinq\Event\SigningConcludedEvent`
extending `OCP\EventDispatcher\Event` that Filinq dispatches when a delegated signing request
...
- GIVEN Filinq dispatches a `SigningConcludedEvent` for a concluded delegated signing request
...
- THEN it can read the subject reference and the full outcome envelope (`signingRequestId`,
  `status`, `signedDocumentRef`, `signers`, `signedAt`) without any further query to Filinq

### Requirement: Listener maps a requested event to SigningService::createRequest

The system SHALL register a listener `OCA\Filinq\EventListener\DocumentSigningRequestedListener`
...
`correlationId`) and SHALL call `OCA\Filinq\Service\SigningService::createRequest($data)` with
...
- GIVEN Filinq is installed and a consumer dispatches a `DocumentSigningRequestedEvent` with a
...
optional and additive: a signing request created without provenance (an internal Filinq request)
...
- GIVEN Filinq's own UI creates a signing request without provenance fields
...
- THEN Filinq dispatches a `SigningConcludedEvent` with `status=signed` carrying the provenance
...
- THEN Filinq dispatches a `SigningConcludedEvent` with `status=declined` respectively
