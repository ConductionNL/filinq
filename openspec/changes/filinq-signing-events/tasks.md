# Tasks: filinq-signing-events

All tasks are `[filinq]`. Mirrors decidesk `decidesk-decision-events`. Backend-only event
contract — no new user-facing string, no UI.

## [filinq] Event contract classes

### T-1. DocumentSigningRequestedEvent (S)

- [x] T-1.1 Add `lib/Event/DocumentSigningRequestedEvent.php`
  (`OCA\Filinq\Event\DocumentSigningRequestedEvent` extends `OCP\EventDispatcher\Event`) with
...
- [x] T-2.1 Add `lib/Event/SigningConcludedEvent.php` (`OCA\Filinq\Event\SigningConcludedEvent`
...
## [filinq] Provenance persistence + conclusion emission
...
- [x] T-7.1 `openspec validate filinq-signing-events --strict` exits 0.
