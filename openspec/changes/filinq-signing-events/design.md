# Design: filinq-signing-events

## Context

The fleet replaced the broken `$registry->call('filinq','createSigningRequest',…)` HTTP-style
delegation with in-process `OCP\EventDispatcher` events, mirroring the just-merged decidesk
`decidesk-decision-events` contract. Filinq already owns the signing lifecycle in
`OCA\Filinq\Service\SigningService`. This change adds only the event surface on top of it.
...
The app's PHP namespace is `OCA\Filinq` (capital D — see `lib/Event/SignerChainCompletedEvent.php`,
`composer.json` PSR-4). The brief wrote `OCA\Filinq`; the real, autoloadable FQCNs are:

- `OCA\Filinq\Event\DocumentSigningRequestedEvent`
- `OCA\Filinq\Event\SigningConcludedEvent`
- `OCA\Filinq\EventListener\DocumentSigningRequestedListener`

The listener lives in `lib/EventListener/` (the app's existing listener directory; e.g.
`ApprovalStepListener`, `FilinqEventListener`), not `lib/Listener/`. The shillinq consumer fix
...
- **Stale OCP stub:** local Psalm/PHPStan may emit phantom errors about `IEventDispatcher` /
  ...
  locally.
- **Synchronous dispatch:** if no listener is registered (Filinq degraded install) the producer
