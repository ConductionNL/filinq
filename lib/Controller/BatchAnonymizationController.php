...
class BatchAnonymizationController extends Controller
{
    ...
    public function batchUpload(): JSONResponse
    {
        try {
            ...
            if (empty($files)) { return new JSONResponse(['error' => $this->l10n->t('No files uploaded')], 400); }
            if (count($files) > $this->stateService->getMaxFiles()) { return new JSONResponse(['error' => $this->l10n->t('Batch size exceeds maximum')], 400); }
    ...
    {
        try { return new JSONResponse($this->extractService->extractNext($batchId)); }
        catch (Exception $e) { return $this->err('Extraction failed', $e); }
    }
    /** @NoAdminRequired @NoCSRFRequired */
    public function batchStatus(string $batchId): JSONResponse
    {
        $batch = $this->stateService->getBatch($batchId);
        if ($batch === null) { return new JSONResponse(['error' => $this->l10n->t('Batch not found')], 404); }
        $ent = 0; $ext = 0;
        foreach ($batch['files'] as $f) { $ent += ($f['entityCount'] ?? 0); if (in_array($f['status'], ['extracted', 'anonymized', 'error'], true)) { $ext++; } }
    ...
    public function batchAnonymize(string $batchId): JSONResponse
    {
        try {
            ...
            if (!is_array($entities) || empty($entities)) { return new JSONResponse(['error' => 'No entities provided'], 400); }
            return new JSONResponse($this->anonService->anonymizeBatch($batchId, $entities));
        } catch (Exception $e) { return $this->err('Anonymization failed', $e); }
    }
    /** @NoAdminRequired @NoCSRFRequired */
    public function batchReport(string $batchId): JSONResponse|DataDownloadResponse
    {
        ...
        } catch (Exception $e) { return $this->err($e->getMessage(), $e); }
    }
    /** @NoAdminRequired @NoCSRFRequired */
    ...
    public function updateProfiles(): JSONResponse
    {
        try {
            ...
            if (!is_array($p['anonymize'] ?? null) || !is_array($p['keep'] ?? null)) { return new JSONResponse(['error' => 'Invalid format'], 400); }
            $this->profileService->saveProfile(['anonymize' => $p['anonymize'], 'keep' => $p['keep']]);
            return new JSONResponse(['message' => 'Profile updated']);
        } catch (Exception $e) { return $this->err('Failed to update profile', $e); }
    }
