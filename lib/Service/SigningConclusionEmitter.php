 ...
 * (filinq-signing-events): deciding whether a concluded signing request is
...
class SigningConclusionEmitter {
	 ...
	 *
	 * Cross-app delegated-signing contract (filinq-signing-events): only
	 * fires for a signing request that carries provenance (`sourceApp` set and
	 * non-empty) — internal Filinq requests emit nothing. The outcome
	...
	public function emitIfDelegated(array $request, string $status, ?string $signedDocumentRef = null): void {
		...
		try {
			...
			$this->logger->info(
				'Filinq: dispatched SigningConcludedEvent',
				[
			...
			$this->logger->error(
				'Filinq: signing request concluded but SigningConcludedEvent dispatch failed',
				[
					'sourceApp' => $sourceApp,
					'status' => $status,
