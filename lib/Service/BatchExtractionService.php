...
class BatchExtractionService
{
    ...
    public function extractNext(string $batchId): array
    {
        ...
        $idx = null;
        foreach ($batch['files'] as $i => $f) { if ($f['status'] === 'uploaded') { $idx = $i; break; } }
        if ($idx === null) {
            $batch['status'] = 'review';
        ...
        $allDone = true;
        foreach ($batch['files'] as $f) { if ($f['status'] === 'uploaded') { $allDone = false; break; } }
        if ($allDone) { $batch['status'] = 'review'; }
        $this->stateService->updateBatch($batchId, $batch);
        $ext = 0;
        foreach ($batch['files'] as $f) { if (in_array($f['status'], ['extracted', 'error'], true)) { $ext++; } }
