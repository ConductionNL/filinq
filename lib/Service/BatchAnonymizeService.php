...
class BatchAnonymizeService
{
    ...
    /**
     * Move the anonymised file from OR's legacy path to the configured output subfolder.
     *
     * On move failure, the file is preserved at the legacy path and a warning is
     * attached to the per-file result. The anonymisation is still counted as successful.
     *
     * @param int                  $sourceFileId NC file ID of the original source file.
     * @param array<string, mixed> $result       Anonymization result from AnonymizationService.
     *
     * @return array{anonymizedFilePath: string|null, warning?: array{code: string, message: string}}
     *
     * @spec openspec/changes/anonymisation-folder-output-folder-layout/tasks.md#task-1
     * @spec openspec/changes/anonymisation-folder-output-folder-layout/tasks.md#task-4
     */
     ...
     * anonymised node's parent is not accessible.
     *
