...
class EntityConsolidationService
{
    ...
    public function consolidateEntities(array $batch, float $minConfidence = 0.0): array
    {
        ...
        foreach ($map as $k => $e) { if ($e['highestConfidence'] < $minConfidence) { $map[$k]['included'] = false; } }
    ...
    private function mergeEntity(array $map, mixed $entity): array
    {
        ...
        if (isset($map[$key])) {
            $map[$key]['fileCount']++;
            if ($conf > $map[$key]['highestConfidence']) { $map[$key]['highestConfidence'] = $conf; }
        } else {
    ...
    {
        try {
            if (!in_array('openregister', $this->appManager->getInstalledApps(), true)) { throw new RuntimeException('OpenRegister not available'); }
